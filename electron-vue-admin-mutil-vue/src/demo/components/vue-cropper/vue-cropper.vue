  ...
  data: function () {
	...
	methods: {
	  ...
	  var nowY = e.clientY ? e.clientY : e.touches ? e.touches[0].clientY : 0
		...
		reload () {
			...
			img.onload = () => {
				...
				this.$nextTick(() => {
					 ...
					 * 初始化点击信息
					 * */
					var box = getScrollOffsets();
					this.clickInfo = {
						//无缩放时的 据左 偏离
						offsetLeft : (this.$refs.cropperImg.width > this.$refs.box.clientWidth) ? this.$refs.cropperImg.width / 2 : 0,
						//无缩放时的 据顶 偏离
						offsetTop : (this.$refs.cropperImg.height > this.$refs.box.clientHeight) ? this.$refs.cropperImg.height / 2 : 0,
						//无缩放时的 中心 x 位置
						centerX : this.$refs.cropperImg.width / 2,
						//无缩放时的 中心 y 位置
						centerY : this.$refs.cropperImg.height / 2,
						//窗体 左 偏离
						boxLeft : box.left,
						//窗体 上 偏离
						boxTop : box.top,
						//框架 左 偏离
						frameTop : this.$refs.cropper.offsetTop,
						//框架 上 偏离
						frameLeft : this.$refs.cropper.offsetLeft
					};
				});
			}
			img.onerror = () => {
		...
		imageClick(e) {
			//
			// var cropperInfo = this.$refs.cropper.getBoundingClientRect();
			// var imgInfo = this.$refs.imgParent.getBoundingClientRect();
			// var ox = e.layerX - (imgInfo.x - cropperInfo.x);
			// var oy = e.layerY - (imgInfo.y - cropperInfo.y);
			// var ret = {
			// 	ox : ox / this.scale,
			// 	oy : oy / this.scale
			// };
            ...
            console.log(ret);
			this.$emit('imageClick',ret);
		},

		// todo 外加方法
		notify(msg,time) {
		    let cTime = (new Date()).getTime();
		    let timer = this.time_ ? 0 : ((cTime - this.time_));
		    this.time_ = cTime;
            this.$dialog.notify({
                mes: '[' + timer + ']' + msg,
                timeout: time || 2000
